...
no real external call. Built against `01_BRD`, `02_PRD` and `03_Epics` in the repository root;
build decisions and spec-conflict adjudications are recorded in **`../PLAN.md`**.
...
possible. They are catalogued with reasoning in `../PLAN.md` §3. The three that change runtime
behaviour:
